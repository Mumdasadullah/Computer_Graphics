...
void landplane()
{
	// Calculate the current position of the plane based on its trajectory
	float t = (250.0 - planeY) / (250.0 - 50.0); // t ranges from 0 to 1
	float landingX = (1 - t) * 250.0 + t * 50.0; // Interpolate X position

	glPushMatrix();
	glTranslatef(landingX, planeY, 0.0);

	// Calculate the angle of rotation for the plane based on its trajectory
	float angle = atan2(250.0 - planeY, landingX - planeX);
	angle = angle * (180.0 / PI);

	// Apply rotation and translation to the plane
	...
	if (planeY > 50.0)
	{
		planeY -= 2.0; // Move the plane vertically downwards
	}

	if (planeX > 50.0)
	{
		planeX -= 2.0; // Move the plane horizontally towards the left
	}
}
